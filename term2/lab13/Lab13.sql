...
BEGIN
	...
	(P_MFR_ID, P_PRODUCT_ID, P_DESCRIPTION, P_PRICE, P_QTY_ON_HAND);
	SELECT count(*) INTO v_product_count FROM products
	WHERE MFR_ID=P_MFR_ID;
	dbms_output.put_line('Количество товаров: '|| v_product_count || '; у продовца: ' || P_MFR_ID);
	EXCEPTION -- нужно обработать ошибки
		WHEN OTHERS THEN
...
BEGIN
	add_product('REI','3345C','Ratchet Link',79.00,210);
END;
...
CREATE OR REPLACE FUNCTION calc_product (
	P_OFFICE integer,
	P_YEAR integer
) RETURN integer IS
	v_count_salereps_no_order integer;
BEGIN
	SELECT count(*) INTO v_count_salereps_no_order -- надо вывести тех у которых нет заказов
	FROM OFFICES ofs
	JOIN SALESREPS ss ON ofs.OFFICE=ss.REP_OFFICE
	JOIN ORDERS os ON os.REP!=ss.EMPL_NUM
	WHERE ofs.OFFICE = P_OFFICE AND P_YEAR=EXTRACT(YEAR FROM os.ORDER_DATE);
	RETURN v_count_salereps_no_order;
END;


BEGIN
	dbms_output.put_line('Количество работников у которых нету заказов:' || calc_product(11, 2007));
END;
